...
def prepare_data(file_path):
    ...
    # 5. Add efficient interaction features
    df['Area_State_Interaction'] = df['Area '] * df.groupby('State')['Yield'].transform('mean')
    df['Crop_Season_Interaction'] = df.groupby(['Crop', 'Season'])['Yield'].transform('mean')
    ...
    feature_columns = [
        ...
        'State_Yield_Mean', 'Area_State_Interaction',
...
def train_model(X, y):
    ...
    model = RandomForestRegressor(
        n_estimators=150,        # Slightly increased trees
